...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.frenzyPower=1;
		Game.clickFrenzy=0;//as long as >0, mouse clicks get 10x more cookies
		...
		Game.milkProgress=0;//you can a little bit for each achievement; 0-1 : milk; 1-2 : chocolate milk; 2-3 : raspberry juice
		...
		Game.mouseCps=function()
		{
			...
			if (Game.Has('Quintillion fingers')) add+=50;
			var num=0;
		...
		Game.ClickCookie=function()
		{
			if (new Date().getTime()-Game.lastClick<1000/250)
			{
				...
				if (Game.autoclickerDetected>=Game.fps*10) Game.Win('Uncanny clicker');
			}
			else
			{
		...
		Game.CalculateGains=function()
		{
			...
			mult+=Game.Has('Zebra cookies')*0.15;
			mult+=Game.Has('Eclipse cookies')*0.15;
			...
			if (Game.Has('Elder Covenant')) mult*=0.9;
		...
		GOLDEN COOKIE
		=======================================================================================*/
		Game.goldenCookie={x:0,y:0,life:0,delay:0,dur:13,toDie:1,wrath:0,chain:0};
		...
		Game.goldenCookie.click=function()
		{
			if (Game.goldenCookie.life>0)
			{
				...
				if (Game.goldenClicks>=77) Game.Win('Leprechaun');
				...
				if (choice=='frenzy')
				{
					Game.frenzy=Game.fps*77;
					Game.frenzyPower=7;
					Game.recalculateGains=1;
					Game.Popup('Frenzy : cookie production x7 for 77 seconds!');
				}
				...
				else if (choice=='blood frenzy')
				{
					...
					Game.recalculateGains=1;
					Game.Popup('Elder frenzy : cookie production x666 for 6 seconds!');
				}
				else if (choice=='clot')
				{
					...
					Game.recalculateGains=1;
					Game.Popup('Clot : cookie production halved for 66 seconds!');
				}
				else if (choice=='click frenzy')
				{
					Game.clickFrenzy=Game.fps*13;
					Game.recalculateGains=1;
					Game.Popup('Click frenzy! Clicking power x10 for 13 seconds!');
				}
				else if (choice=='chain cookie')
				{
					...
					if ((Math.random()<0.05 || Game.goldenCookie.chain>12 || moni>=Game.cookies*1.5) && Game.goldenCookie.chain>4) Game.goldenCookie.chain=0;
				}
			}
		}
		...
		{
			var str='';
			if (time>Game.fps*60*60*2) str=Beautify(time/(Game.fps*60*60))+' hours';
			else if (time>Game.fps*60*60) str='1 hour';
			else if (time>Game.fps*60*2) str=Beautify(time/(Game.fps*60))+' minutes';
			else if (time>Game.fps*60) str='1 minute';
			else if (time>Game.fps*2) str=Beautify(time/(Game.fps))+' seconds';
			else if (time>Game.fps) str='1 second';
			return str;
		}
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='prefs')
			{
				...
				'<div class="listing"><a onclick="Game.WriteSave();">Save</a><label>Save manually (the game autosaves every 60 seconds)</label></div>'+
				'<div class="listing"><a onclick="Game.ExportSave();">Export save</a><a onclick="Game.ImportSave();">Import save</a></div>'+
				...
				'<div class="listing"><a onclick="Game.Reset();" class="warning">Reset</a><label>Reset your game (but keep your achievements)</label></div>'+
				'<div class="listing"><a onclick="Game.HardReset();" class="warning">Hard-reset</a><label>Delete all your progress, including your achievements</label></div>'+
				...
				'<div class="subsection">'+
				'<div class="title">General</div>'+
				'<div class="listing"><a onclick="Game.Reset();" class="warning">Reset</a><b>Reset your game (but keep your achievements)<a onclick="Game.WriteSave();">Save</a><a onclick="Game.ExportSave();">Export save</a><a onclick="Game.ImportSave();">Import save</a><a onclick="Game.HardReset();" style="float:right;" class="warning">Hard-reset</a></div>'+
				'<div class="listing"><span class="warning" style="font-size:12px;">[Note : importing saves from earlier versions than 1.0 will be disabled beyond September 1st, 2013.]</span></div>'+
				'<div class="title">Settings</div>'+
				'<div class="listing">'+Game.WriteButton('particles','particlesButton','Particles ON','Particles OFF')+Game.WriteButton('numbers','numbersButton','Numbers ON','Numbers OFF')+Game.WriteButton('milk','milkButton','Milk ON','Milk OFF')+'</div>'+
			...
			else if (Game.onMenu=='stats')
			{
				...
				if (Game.milkProgress>=2.5) milkName='raspberry juice';
				...
				'<div class="listing"><b>Cookies per second :</b> '+Beautify(Game.cookiesPs,1)+' <small>(multiplier : '+Beautify(Game.globalCpsMult*100,1)+'%)</small></div>'+
		...
		Game.Object=function(name,commonName,desc,pic,icon,background,price,cps,drawFunction,buyFunction)
		{
			...
			this.setSpecial=function(what)
			{
				...
				else if (what==0) this.onSpecial=0;
				this.refresh();
			}
		...
		new Game.Object('Farm','farm|farms|harvested','Grows cookie plants from cookie seeds.','farm','farmIcon','farmBackground',500,function(){
			...
			if (this.amount>=5) Game.Unlock('Farmer grandmas');
		...
		new Game.Object('Factory','factory|factories|mass-produced','Produces large quantities of cookies.','factory','factoryIcon','factoryBackground',3000,function(){
			...
			if (this.amount>=5) Game.Unlock('Worker grandmas');
		...
		new Game.Object('Mine','mine|mines|mined','Mines out cookie dough and chocolate chips.','mine','mineIcon','mineBackground',10000,function(){
			...
			if (this.amount>=5) Game.Unlock('Miner grandmas');
		...
		new Game.Object('Shipment','shipment|shipments|shipped','Brings in fresh cookies from the cookie planet.','shipment','shipmentIcon','shipmentBackground',40000,function(){
			...
			if (this.amount>=5) Game.Unlock('Cosmic grandmas');
		...
		new Game.Object('Alchemy lab','alchemy lab|alchemy labs|transmuted','Turns gold into cookies!','alchemylab','alchemylabIcon','alchemylabBackground',200000,function(){
			...
			if (this.amount>=5) Game.Unlock('Transmuted grandmas');
		...
		new Game.Object('Portal','portal|portals|retrieved','Opens a door to the Cookieverse.','portal','portalIcon','portalBackground',1666666,function(){
			...
			if (this.amount>=5) Game.Unlock('Altered grandmas');
		...
		new Game.Object('Time machine','time machine|time machines|recovered','Brings cookies from the past, before they were even eaten.','timemachine','timemachineIcon','timemachineBackground',123456789,function(){
			...
			if (this.amount>=5) Game.Unlock('Grandmas\' grandmas');
		...
		new Game.Upgrade('Specialized chocolate chips','[Research]<br>All cookie income <b>+1%</b>.<q>Computer-designed chocolate chips. Computer chips, if you will.</q>',10000000000,[0,9],function(){Game.SetResearch('Designer cocoa beans');});
		new Game.Upgrade('Designer cocoa beans','[Research]<br>All cookie income <b>+2%</b>.<q>Now more aerodynamic than ever!</q>',20000000000,[1,9],function(){Game.SetResearch('Ritual rolling pins');});
		...
		new Game.Upgrade('Underworld ovens','[Research]<br>All cookie income <b>+3%</b>.<q>Powered by science, of course!</q>',80000000000,[3,9],function(){Game.SetResearch('One mind');});
		...
		new Game.Upgrade('Exotic nuts','[Research]<br>All cookie income <b>+4%</b>.<q>You\'ll go crazy over these!</q>',320000000000,[5,9],function(){Game.SetResearch('Communal brainsweep');});
		...
		new Game.Upgrade('Arcane sugar','[Research]<br>All cookie income <b>+5%</b>.<q>Tastes like insects, ligaments, and molasses.</q>',1280000000000,[7,9],function(){Game.SetResearch('Elder Pact');});
		new Game.Upgrade('Elder Pact','[Research]<br>Each grandma gains <b>+1 base CpS for each 20 portals</b>.<div class="warning">Note : this is a bad idea.</div><q>squirm crawl slither writhe<br>today we rise</q>',2560000000000,[8,9],function(){Game.elderWrath=3;Game.Unlock('Elder Pledge');});
		new Game.Upgrade('Elder Pledge','[Repeatable]<br>Contains the wrath of the elders, at least for a while.',1,[9,9],function()
		{
			Game.elderWrath=0;
			Game.pledges++;
			Game.pledgeT=Game.fps*60*30;
			Game.Upgrades['Elder Pledge'].basePrice=Math.pow(8,Math.min(Game.pledges+2,13));
		...
		order=10000;
		new Game.Upgrade('Eclipse cookies','All cookie income <b>+15%</b>.<q>Look to the cookie.</q>',9999999999999,[0,4]);
		new Game.Upgrade('Zebra cookies','All cookie income <b>+15%</b>.',9999999999999,[1,4]);
		
		order=100;
		new Game.Upgrade('Quintillion fingers','The mouse and cursors gain <b>+50</b> cookies for each non-cursor object owned.<q>man, just go click click click click click, it’s real easy, man.</q>',50000000000000,[3,6]);
		...
		order=15000;
		new Game.Upgrade('Elder Covenant','[Togglable]<br>Puts a permanent end to the elders\' wrath, at the price of 10% of your CpS.',6666666666666,[8,9],function()
		{
			Game.elderWrath=0;
			Game.pledgeT=1;
		...
		new Game.Upgrade('Revoke Elder Covenant','[Togglable]<br>You will get 10% of your CpS back, but the grandmatriarchs will return.',6666666666666,[8,9],function()
		{
			Game.elderWrath=1;
			Game.pledgeT=1;
		...
		*/
		...
		order=30001;
		new Game.Achievement('Uncanny clicker','Beat the clicking speed world record.<q>Well I\'ll be!</q>',[12,0],3);
		...
		order=10000;
		new Game.Achievement('Leprechaun','Click <b>77 golden cookies</b>.<q>You should really go to bed.</q>',[10,1],3);
		order=30000;
		new Game.Achievement('True Neverclick','Make <b>1 million</b> cookies with <b>no</b> cookie clicks.',[12,0],3);
		...
		{
			if (Game.Has('Elder Covenant')) Game.elderWrath=0;
			if (Game.pledgeT>0)//if the pledge is active, lower it
			{
				Game.pledgeT--;
			...
			{
				if (Game.pledges>0 && Math.random()<0.0001 && Game.elderWrath<3 && !Game.Has('Elder Covenant')) Game.elderWrath++;//have we already pledged ? make the elder wrath shift between different stages
			}
		...
		Game.Objects['Factory'].special=function()
		{
			...
			this.dungeon=new Game.Dungeon('Factory');
		
		}
		//var test=new Game.Dungeon('Factory');
		//you guys will have to be a little patient, this is a huge one
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps*3)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
